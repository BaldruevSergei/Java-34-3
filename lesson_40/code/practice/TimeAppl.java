...
//          **Парсинг даты из строки**
        ...
        df = DateTimeFormatter.ofPattern("dd MMMM yy", Locale.ENGLISH); // задали другой формат отображения
        System.out.println(gagarin.format(df));
        ...
        LocalDate date1 = dateParce(str); // делаем метод для конвертации строик в дату
    ...
    private static  LocalDate dateParce(String date) { // строка на входе
        DateTimeFormatter df = DateTimeFormatter.ofPattern("[yyyy-MM-dd][dd/MM/yyyy][dd.MM.yyyy][dd:MM:yyyy]"); // определяем
        return  LocalDate.parse(date, df);// преобразование в дату типа Iso
    }
}
